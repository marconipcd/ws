...
public class PNGPDFTransformerHelperUtil {
	
	/* Ainda precisa de um nome melhor!! */
	public static final float IMAGEM_BOLETO_WIDTH = 595.0f;
	public static final float IMAGEM_BOLETO_HEIGHT = 842.0f;
	public static final double BOLETO_TEMPLATE_SCALE = 1 / 2d;

	private TextWriter writer;
	private URL imagemTitulo;

	public PNGPDFTransformerHelperUtil(TextWriter writer) {
		super();
		this.writer = writer;
		this.imagemTitulo = PNGPDFTransformerHelperUtil.class.getResource("/com/digital/opuserp/util/boletos/img/template_duplo.png");
	}
...
//		geraEImprimeCodigoDeBarras(boleto);
	...
	private BufferedImage toBufferedImage(Image image, int type) {
		return BufferedImageGeneratorUtil.generateBufferedImageFor(image, type);
	}
	...
	private String formatDate(Calendar date) {
		return FormatadorDeBoletoUtil.formataData(date);
	}

	private void geraTemplateComOFundoDoBoleto(Boleto boleto) {
		try {
			this.writer.writeImage(0, 0, imageFor(imagemTitulo), IMAGEM_BOLETO_WIDTH, IMAGEM_BOLETO_HEIGHT);
	...
	private void imprimeReciboDeSacado(Boleto boleto) {
		...
		this.writer.write(377, LINHA2, FormatadorDeBoletoUtil.formataValor(boleto.getValorBoleto().doubleValue()));
	...
	private void imprimeDadosDoBoletoInclusiveValor(Boleto boleto) {
		...
		this.writer.write(430, LINHA8, FormatadorDeBoletoUtil.formataValor(boleto.getValorBoleto().doubleValue()));
	}
