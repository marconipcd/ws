...
public class SimplesNacionalUtil  extends Window {
	
	JPAContainer<SimplesNacional> container;
	...
	public SimplesNacionalUtil(boolean modal, boolean center){
		
		
		super("Selecione Simples Nacional");
	...
	public JPAContainer<SimplesNacional> buildJpaContainer(){
		container = JPAContainerFactory.make(SimplesNacional.class, ConnUtil.getEntity());	
...
//		}
//				
		container.sort(new String[]{"simples_nacional_cod"},new boolean[]{true});
		return container;
	}
	...
	public Table buildTb(){
		...
		tb.setColumnHeader("simples_nacional_cod", "CSOSN Código");
		tb.setColumnHeader("referencia", "Referência");
		
		tb.setVisibleColumns(new Object[]{"simples_nacional_cod","referencia"});
			...
			public void itemClick(ItemClickEvent event) {
				if(event.isDoubleClick()){
					...
					if(item.getItemProperty("id").toString()!=null){
						simples = SimplesNacionalDAO.find((Integer)item.getItemProperty("id").getValue());										
					}
					fireEvent(new CstEvent(getUI(), simples));
					close();
...
//		container.addContainerFilter(Filters.eq("grupoId", 21));	
		...
		btSalvar = new Button("Ok", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tb.getValue()!= null){
					...
					SimplesNacional simples = SimplesNacionalDAO.find((Integer)item.getItemProperty("id").getValue());				
	...
	public static class CstEvent extends Event{
		...
		private SimplesNacional simples;
		
		public CstEvent(Component source, SimplesNacional simples) {
			super(source);		
			this.simples  = simples;
		}

		public SimplesNacional getCstSimples() {
