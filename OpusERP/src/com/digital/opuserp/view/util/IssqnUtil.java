...
public class IssqnUtil extends Window {
	...
	public IssqnUtil(boolean modal, boolean center, Integer codEmpresa){
		
		
		
		super("Selecione um Issqn");
	...
	public JPAContainer<Issqn> buildJpaContainer(){
		container = JPAContainerFactory.make(Issqn.class, ConnUtil.getEntity());
//		container.addContainerFilter(Filters.eq("empresa_id", codEmpresa));
		...
		tb.addListener(new ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				if(event.isDoubleClick()){
					...
					EntityItem<Issqn> entityItem = (EntityItem<Issqn>)tb.getItem(tb.getValue());
					Issqn issqn = entityItem.getEntity();
															
					fireEvent(new IssqnUtilEvent(getUI(), issqn));
					close();
	...
	public void addListerner(IssqnUtilListerner target){
		try {
			Method method = IssqnUtilListerner.class.getDeclaredMethod("onSelected",IssqnUtilEvent.class);
			addListener(IssqnUtilEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(IssqnUtilListerner target){
		removeListener(IssqnUtilEvent.class, target);
	}
	public static class IssqnUtilEvent extends Event{

		private Issqn issqn;
		
		public IssqnUtilEvent(Component source, Issqn issqn) {
			super(source);		
			this.issqn  = issqn;
		}
	...
	public interface IssqnUtilListerner{
		public void onSelected(IssqnUtilEvent event);
	}
}
