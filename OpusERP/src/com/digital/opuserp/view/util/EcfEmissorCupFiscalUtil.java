...
public class EcfEmissorCupFiscalUtil  extends Window {
	
	JPAContainer<EcfEmissorCupFiscal> container;
	...
	public EcfEmissorCupFiscalUtil(boolean modal, boolean center){
		
		
		super("Selecione um ECF");
	...
	public JPAContainer<EcfEmissorCupFiscal> buildJpaContainer(){
		container = JPAContainerFactory.make(EcfEmissorCupFiscal.class, ConnUtil.getEntity());	
...
//				
	...
	public Table buildTb(){
		...
		tb.setColumnHeader("ecf_emissor_cod", "ECF Código");
		tb.setColumnHeader("referencia", "Referência");
		
		tb.setVisibleColumns(new Object[]{"ecf_emissor_cod","referencia"});
		tb.addListener(new ItemClickListener() {
			
			EcfEmissorCupFiscal ecf;
			...
			public void itemClick(ItemClickEvent event) {
				if(event.isDoubleClick()){
					...
					if(item.getItemProperty("id").toString()!=null){
						ecf = EcfEmissorCupFiscalDAO.find((Integer)item.getItemProperty("id").getValue());										
					}
					fireEvent(new CstEvent(getUI(), ecf));
					close();
...
//		container.addContainerFilter(Filters.eq("grupoId", 21));	
		...
		btSalvar = new Button("Ok", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tb.getValue()!= null){
					...
					EcfEmissorCupFiscal ecf = EcfEmissorCupFiscalDAO.find((Integer)item.getItemProperty("id").getValue());				
	...
	public static class CstEvent extends Event{
		...
		private EcfEmissorCupFiscal scf;
		
		public CstEvent(Component source, EcfEmissorCupFiscal scf) {
			super(source);		
			this.scf  = scf;
		}

		public EcfEmissorCupFiscal getCstEcf() {
