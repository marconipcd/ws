...
public class ProdutoUtil  extends Window {
	...
	public ProdutoUtil(boolean modal, boolean center, Cliente c){
	...
	public ProdutoUtil(boolean modal, boolean center, Cliente c, TipoItemProduto itemProduto){
	...
	public ProdutoUtil(boolean modal, boolean center){
	...
	public ProdutoUtil(boolean modal, boolean center, boolean ordem_servico){
	...
	public ProdutoUtil(boolean modal, boolean center,Integer CodComp){
	...
	public JPAContainer<Produto> buildJpaContainer(){
		...
		if(ordem_servico){
			container.addContainerFilter(Filters.eq("tipo_item", configOse.getTipo_item_default()));
		}
		
		if(itemProduto != null){
			container.addContainerFilter(Filters.eq("tipo_item", itemProduto));
		}
...
//					Item item = tbProdutos.getItem(tbProdutos.getValue());
					...
					if(produto != null && produto.getPreco_promocional() != null && produto.getPreco_promocional() > 0){
						return "R$ "+Real.formatDbToString(String.valueOf(produto.getPreco_promocional()));
					}else{
		...
		tbProdutos.setVisibleColumns(new Object[]{"id","gTin","nome","valorVenda","qtdEstoque","qtdEstoqueDeposito","garantia"});
		...
		btSalvar = new Button("Ok", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tbProdutos.getValue()!= null){
					Item item = tbProdutos.getItem(tbProdutos.getValue());			
					Produto material = ProdutoDAO.find(Integer.parseInt(item.getItemProperty("id").toString()));
