...
public class EditarMaterialEditor extends Window implements GenericEditor{
	...
	public EditarMaterialEditor(Item item, String title, boolean modal){
		this.item = item;
		...
		vlRoot.addComponent(new FormLayout(){					
				{
					...
					if(item.getItemProperty("regime").getValue().equals("COMODATO")){					
						...
						txtMacOpcional.setStyleName("caption-align-editar-material");						
				...
				try {	
					...
					if(item.getItemProperty("regime").getValue().equals("COMODATO")){	
						...
						if(txtMacOpcional.getValue() != null && !txtMacOpcional.getValue().equals("")){
							...
							item.getItemProperty("onu_serial").setValue(null);
						}
