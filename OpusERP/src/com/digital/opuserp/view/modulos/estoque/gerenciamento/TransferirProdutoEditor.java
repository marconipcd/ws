...
public class TransferirProdutoEditor extends Window implements GenericEditor {
	...
	public TransferirProdutoEditor(Item item, String title, boolean modal){
		this.item = item;
		...
		btCancelar = new Button("Cancelar", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(!fieldGroup.isModified()){
					fieldGroup.discard();				
					fireEvent(new TransferirProdutoEvent(getUI(), item, false));					
				}else{
	...
	private void concluirCadastro(){
		...
		if(produtoSelecionado != null && txtQtd.getValue() != null && validarQtd){
			...
			try {		
				...
				item.getItemProperty("tipo_transferencia").setValue("TRANSFERENCIA");
				...
				fireEvent(new TransferirProdutoEvent(getUI(), item, true));
	...
	public void addListerner(TransferirProdutoListener target){
		try {
			Method method = TransferirProdutoListener.class.getDeclaredMethod("onClose", TransferirProdutoEvent.class);
			addListener(TransferirProdutoEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(TransferirProdutoListener target){
		removeListener(TransferirProdutoEvent.class, target);
	}
	public static class TransferirProdutoEvent extends Event{
		
		private Item item;
		private boolean confirm;
		
		public TransferirProdutoEvent(Component source, Item item, boolean confirm) {
			super(source);
			this.item = item;
	...
	public interface TransferirProdutoListener extends Serializable{
