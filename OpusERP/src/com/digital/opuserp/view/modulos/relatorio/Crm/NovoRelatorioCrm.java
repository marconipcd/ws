...
public class NovoRelatorioCrm extends Window {
	...
	public NovoRelatorioCrm(String title, boolean modal){
	...
	public NovoRelatorioCrm(String title, boolean modal,String tipo,String orientacao,String ordenacao,String resumo, List<SearchParameters> listaParametros){
	...
	public void buildLayout(){
		...
		vlRoot.addComponent(
			new FormLayout(){					
			{
				...
				cbOrdenacao.addItem("Crm Assuntos");
				cbOrdenacao.addItem("Contato");
				cbOrdenacao.addItem("Data Agendado");
				cbOrdenacao.addItem("Data Conclusão");
				cbOrdenacao.addItem("Hora Agendado");
				cbOrdenacao.addItem("Status"); 			
				cbOrdenacao.addItem("Operador Tratamento"); 
				cbOrdenacao.addItem("Data Inicio Tratamento"); 
				cbOrdenacao.addItem("Tempo Total Tratamento"); 
				cbOrdenacao.addItem("Motivo Reagendamento");
		...
		vlRoot.addComponent(
				new FormLayout(){					
				{
					...
					cbResumo.addItem("Crm Assuntos");
					cbResumo.addItem("Contato");
					cbResumo.addItem("Data Agendado");
					cbResumo.addItem("Data Conclusão");
					cbResumo.addItem("Hora Agendado");
					cbResumo.addItem("Status");  
					cbResumo.addItem("Operador Tratamento"); 
					cbResumo.addItem("Data Inicio Tratamento"); 
					cbResumo.addItem("Tempo Total Tratamento"); 
					cbResumo.addItem("Motivo Reagendamento"); 
	    ...
	    t = ts_principal.addTab(new FormLayout(){
	    		{
					...
					if(listaParametros != null && listaParametros.size() >0){
						...
						for (SearchParameters s : listaParametros) {
							...
							if(selectHeader(s.getCampo()).equals("Status")){							
								hlFiltro.addComponent(buildcbValorFiltro(s.getValor(),selectHeader(s.getCampo()),true));
							}else if(selectHeader(s.getCampo()).equals("Data Agendado") || selectHeader(s.getCampo()).equals("Data Conclusão")){
	...
	private ComboBox buildcbValorFiltro(String v, String coluna,final boolean editing) {	
		...
		if(coluna.equals("Status")){
			cBValorFiltro.addItem("PENDENTE");
			cBValorFiltro.addItem("EM TRATAMENTO");
			cBValorFiltro.addItem("EFETUADO");	
			
		}else if(coluna.equals("Crm Assuntos")){		
			
			List<CrmAssunto> CrmAssunto = CrmAssuntosDAO.getAssuntos();
			
			for (CrmAssunto assunto2 : CrmAssunto) {
				cBValorFiltro.addItem(assunto2.getNome());
			}	
			
		}else if(coluna.equals("Setor")){
					
			List<Setores> setor = SetoresDAO.getsetores();
			
			for (Setores setor2 : setor) {
				cBValorFiltro.addItem(setor2.getNome());
			}	
		}
...
//		cbFiltro.focus();
		...
		cbFiltro.addItem("Crm Assuntos");
		cbFiltro.addItem("Contato");
		cbFiltro.addItem("Data Cadastro");
		cbFiltro.addItem("Data Agendado");
		cbFiltro.addItem("Data Conclusão");
		cbFiltro.addItem("Status");  
		cbFiltro.addItem("Operador Tratamento"); 
		cbFiltro.addItem("Data Inicio Tratamento"); 
		cbFiltro.addItem("Tempo Total Tratamento"); 
		cbFiltro.addItem("Motivo Reagendamento");
		cbFiltro.addItem("Contrato");
		cbFiltro.addItem("Plano");
		cbFiltro.addItem("Concentrador");
		cbFiltro.addItem("Conteudo");
				...
				if(v == null){
					...
					if(cbOperador != null){
						...
						if(type == String.class){					
							...
							if(coluna.equals("Status") || coluna.equals("Setor") || coluna.equals("Crm Assuntos")){
		...
		btAdd = new Button("Add",new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if(event.getButton().getCaption().toString().equals("Add")){
						if(cbFiltro.getValue().toString().equals("Status") || cbFiltro.getValue().toString().equals("Crm Assuntos") || cbFiltro.getValue().toString().equals("Setor")){
							cbFiltro.setEnabled(false);
							cbOperador.setEnabled(false);
							...
							hlFiltro.addComponent(buildBtAdd(false, null));
							vlFiltros.addComponent(hlFiltro);		
						}else if(cbFiltro.getValue().toString().equals("Data Cadastro") ||cbFiltro.getValue().toString().equals("Data Agendado") || cbFiltro.getValue().toString().equals("Data Conclusão") || cbFiltro.getValue().toString().equals("Data Inicio Tratamento")){
							cbFiltro.setEnabled(false);
							cbOperador.setEnabled(false);
					...
					fireEvent(new AddFiltroCrmEvent(getUI(), listaParametros));
				}
		...
		btSalvar = new Button("OK", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(cbOrdenacao.isValid() && cbTipoTabela.isValid() && cbResumo.isValid() && listaParametros.size() > 0){
					fireEvent(new RelatorioCrmEvent(getUI(), true, listaParametros, cbTipoTabela.getValue().toString(), cbOrdenacao.getValue().toString(), cbOrientacao.getValue().toString(), cbResumo.getValue().toString()));		
					close();
				}else{
	...
	public Button buildBtCancelar() {
		btCancelar = new Button("Cancelar", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				fireEvent(new RelatorioCrmEvent(getUI(), false, null,null,null, null, null));
				close();	
			}
		});
	...
	public void addListerner(RelatorioCrmListerner target){
		try {
			Method method = RelatorioCrmListerner.class.getDeclaredMethod("onClose", RelatorioCrmEvent.class);
			addListener(RelatorioCrmEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(RelatorioCrmListerner target){
		removeListener(RelatorioCrmEvent.class, target);
	}
	
	
	public void addListerner(AddFiltroCrmListerner target){
		try {
			Method method = AddFiltroCrmListerner.class.getDeclaredMethod("onClose", AddFiltroCrmEvent.class);
			addListener(AddFiltroCrmEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(AddFiltroCrmListerner target){
		removeListener(AddFiltroCrmEvent.class, target);
	}
	
	
	public static class AddFiltroCrmEvent extends Event{

		private List<SearchParameters> filtros;
		
		
		public AddFiltroCrmEvent(Component source, List<SearchParameters> filtros) {
			super(source);
	...
	public interface AddFiltroCrmListerner extends Serializable{
		public void onClose(AddFiltroCrmEvent event);
	}
	
	
	public static class RelatorioCrmEvent extends Event{
		...
		public RelatorioCrmEvent(Component source, boolean confirm, List<SearchParameters> parametros, String tipo, String ordenacao, 
	...
	public interface RelatorioCrmListerner extends Serializable{
	...
	public void adicionarFiltro(){
		...
		for(SearchParameters sp:listaParametros){
				...
				if(sp.getOperador().equals("IGUAL")){
					...
					if(container.getType(sp.getCampo()) == Date.class && !sp.getCampo().equals("data_agendado")){
