...
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.pis.PisEditor.PisEvent;
...
public class PisView extends VerticalLayout implements GenericView {
	...
	public JPAContainer<Pis> buildContainer(){
		container = JPAContainerFactory.makeBatchable(Pis.class, ConnUtil.getEntity());
	...
	public Table buildTbGeneric() {
		...
		tb.setColumnHeader("cst", "Base de Cálculo");		
		tb.setColumnHeader("tipo_pis", "Aliquota");		
		tb.setColumnHeader("base_calculo", "Item Lista Serviço");
		tb.setColumnHeader("aliquota", "Situação Tributária");
					
		tb.setVisibleColumns(new Object[] {"id","descricao","cst","tipo_pis","base_calculo","aliquota"});		
	...
	tb.addItemClickListener(new ItemClickEvent.ItemClickListener() {
		...
		public void itemClick(ItemClickEvent event) {
			if(event.isDoubleClick()  && tb.getValue() != null){
				...
				PisEditor cEditor = new PisEditor(tb.getItem(tb.getValue()), "Editar Pis", true);
				cEditor.addListerner(new PisEditor.PisListerner() {
	...
	public Button BuildbtTransferencia() {
		btNovo = new Button("Novo", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
					...
					final BeanItem<Pis> newItem = new BeanItem<Pis>(new Pis());
								
					PisEditor cEditor = new PisEditor(newItem, "Cadastrar Pis", true);
					cEditor.addListerner(new PisEditor.PisListerner() {
						...
						public void onClose(PisEvent event) {
							if(event.isConfirm()){
								...
								try {
									Pis cAdd = newItem.getBean();									
									container.addEntity(cAdd);	
									container.commit();
									hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
									Notify.Show("Pis Cadastrado com Sucesso!", Notify.TYPE_SUCCESS);
								} catch (Exception e) {
									e.printStackTrace();																
				...
				cEditor.addListerner(new PisEditor.PisListerner() {
					...
					public void onClose(PisEvent event) {
						if(event.isConfirm()){
							container.commit(); 
							hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
							Notify.Show("Pis Alterado com Sucesso!", Notify.TYPE_SUCCESS);
						}
					}
				});
	...
	public Button BuildbtCancelar() {
		btExcluir = new Button("Excluir", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				GenericDialog gDialog = new GenericDialog("Confirme para Continuar!", "Você deseja realmente Excluir o  Pis Selecionado?", true, true);
					gDialog.addListerner(new GenericDialog.DialogListerner() {
						...
						public void onClose(DialogEvent event) {
							if(event.isConfirm()){
								...
								container.commit();
								hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
								Notify.Show("Pis Excluido com Sucesso!", Notify.TYPE_SUCCESS);
							}
						}
					});
