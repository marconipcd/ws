...
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.cofins_st.CofinsStEditor;
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.cofins_st.CofinsStEditor.CofinsstEvent;
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.icms.IcmsEditor.IcmsEvent;
...
public class IcmsView extends VerticalLayout implements GenericView {
	...
	public JPAContainer<Icms> buildContainer(){
		container = JPAContainerFactory.makeBatchable(Icms.class, ConnUtil.getEntity());
	...
	public Table buildTbGeneric() {
		...
		tb.setColumnHeader("modalidade_base_calculo_icms", "Modalidade Base Cálculo ICMS");		
		tb.setColumnHeader("aliquota_icms", "Aliquota ICMS");
		
		tb.setVisibleColumns(new Object[] {"id","descricao","cst","modalidade_base_calculo_icms","aliquota_icms"});
	...
	tb.addItemClickListener(new ItemClickEvent.ItemClickListener() {
		...
		public void itemClick(ItemClickEvent event) {
			if(event.isDoubleClick()  && tb.getValue() != null){
				...
				IcmsEditor cEditor = new IcmsEditor(tb.getItem(tb.getValue()), "Editar ICMS", true);
				cEditor.addListerner(new IcmsEditor.IcmsListerner() {
	...
	public Button BuildbtTransferencia() {
		btNovo = new Button("Novo", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
					...
					final BeanItem<Icms> newItem = new BeanItem<Icms>(new Icms());
								
					IcmsEditor cEditor = new IcmsEditor(newItem, "Cadastrar ICMS", true);
					cEditor.addListerner(new IcmsEditor.IcmsListerner() {
						...
						public void onClose(IcmsEvent event) {
							if(event.isConfirm()){
								...
								try {
									Icms cAdd = newItem.getBean();									
									container.addEntity(cAdd);	
									container.commit();
									hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
									Notify.Show("ICMS Cadastrado com Sucesso!", Notify.TYPE_SUCCESS);
								} catch (Exception e) {
									e.printStackTrace();																
				...
				cEditor.addListerner(new IcmsEditor.IcmsListerner() {
					...
					public void onClose(IcmsEvent event) {
						if(event.isConfirm()){
							container.commit(); 
							hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
							Notify.Show("ICMS Alterado com Sucesso!", Notify.TYPE_SUCCESS);
						}
					}
				});
