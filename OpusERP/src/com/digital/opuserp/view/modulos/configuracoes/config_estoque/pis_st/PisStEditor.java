...
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.issqn.IssqnEditor.IssqnListerner;
...
public class PisStEditor  extends Window implements GenericEditor {
	...
	public PisStEditor(Item item, String title, boolean modal){
		this.item = item;
...
//		buildLayout();	
										...
										addComponent(new FormLayout() {
											{
												...
												TextField txtPis = (TextField)fieldGroup.buildAndBind("Tipo Pis", "tipo_pis");				
												txtPis.setWidth("130px");				
												txtPis.setStyleName("caption-align");
												txtPis.setNullRepresentation("");
												txtPis.setMaxLength(200);
												
												addComponent(txtPis);
												setExpandRatio(txtPis, 2);		
		...
		btSalvar = new Button("OK", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(fieldGroup.isValid()){
					try {
						fieldGroup.commit();				
						fireEvent(new PisStEvent(getUI(), item, true));
		...
		btCancelar = new Button("Cancelar", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(!fieldGroup.isModified()){
					fieldGroup.discard();				
					fireEvent(new PisStEvent(getUI(), item, false));
	...
	public void addListerner(PisStListerner target){
		try {
			Method method = PisStListerner.class.getDeclaredMethod("onClose", PisStEvent.class);
			addListener(PisStEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(PisStListerner target){
		removeListener(PisStEvent.class, target);
	}
	public static class PisStEvent extends Event{
		
		private Item item;
		private boolean confirm;
		
		public PisStEvent(Component source, Item item, boolean confirm) {
			super(source);
			this.item = item;
	...
	public interface PisStListerner extends Serializable{
