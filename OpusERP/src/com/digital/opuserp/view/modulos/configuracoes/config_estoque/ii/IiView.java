...
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.icms.IcmsView;
import com.digital.opuserp.view.modulos.configuracoes.config_estoque.ii.IiEditor.IiEvent;
...
public class IiView extends VerticalLayout implements GenericView {
	...
	public JPAContainer<Ii> buildContainer(){
		container = JPAContainerFactory.makeBatchable(Ii.class, ConnUtil.getEntity());
	...
	public Table buildTbGeneric() {
		...
		tb.setVisibleColumns(new Object[] {"id","descricao","base_calculo","aliquota"});
	...
	tb.addItemClickListener(new ItemClickEvent.ItemClickListener() {
		...
		public void itemClick(ItemClickEvent event) {
			if(event.isDoubleClick()  && tb.getValue() != null){
				...
				IiEditor cEditor = new IiEditor(tb.getItem(tb.getValue()), "Editar Ii", true);
				cEditor.addListerner(new IiEditor.IiListerner() {
	...
	public Button BuildbtTransferencia() {
		btNovo = new Button("Novo", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
					...
					final BeanItem<Ii> newItem = new BeanItem<Ii>(new Ii());
								
					IiEditor cEditor = new IiEditor(newItem, "Cadastrar Ii", true);
					cEditor.addListerner(new IiEditor.IiListerner() {
						...
						public void onClose(IiEvent event) {
							if(event.isConfirm()){
								...
								try {
									Ii cAdd = newItem.getBean();									
									container.addEntity(cAdd);	
									container.commit();
									hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
									Notify.Show("Ii Cadastrado com Sucesso!", Notify.TYPE_SUCCESS);
								} catch (Exception e) {
									e.printStackTrace();																
				...
				cEditor.addListerner(new IiEditor.IiListerner() {
					...
					public void onClose(IiEvent event) {
						if(event.isConfirm()){
							container.commit(); 
							hlFloat.replaceComponent(lbRegistros, buildLbRegistros());
							Notify.Show("Ii Alterado com Sucesso!", Notify.TYPE_SUCCESS);
						}
					}
				});
