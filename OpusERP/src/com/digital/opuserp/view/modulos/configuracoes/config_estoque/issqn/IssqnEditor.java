...
public class IssqnEditor  extends Window implements GenericEditor {
	...
	public IssqnEditor(Item item, String title, boolean modal){
		this.item = item;
...
//		buildLayout();	
										...
										addComponent(new FormLayout() {
											{
												...
												TextField txtItemListaServico = (TextField)fieldGroup.buildAndBind("Item Lista Serviço", "item_lista_servico");				
												txtItemListaServico.setWidth("130px");				
												txtItemListaServico.setStyleName("caption-align");
												txtItemListaServico.setNullRepresentation("");
												txtItemListaServico.setMaxLength(200);
												
												addComponent(txtItemListaServico);
												setExpandRatio(txtItemListaServico, 2);		
												...
												addComponent(new FormLayout() {
													{
														...
														ComboBox cbSituacaoTributaria = new ComboBox("Situação Tributária");				
														cbSituacaoTributaria.setWidth("130px");				
														cbSituacaoTributaria.setStyleName("caption-align");
														cbSituacaoTributaria.setNullSelectionAllowed(false);
														cbSituacaoTributaria.setTextInputAllowed(false);
														cbSituacaoTributaria.addItem("Normal");
														cbSituacaoTributaria.addItem("Retida");
														cbSituacaoTributaria.addItem("Substituta");
														cbSituacaoTributaria.addItem("Isenta");
																												
														addComponent(cbSituacaoTributaria);
														fieldGroup.bind(cbSituacaoTributaria, "situacao_tributaria");
		...
		btSalvar = new Button("OK", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(fieldGroup.isValid()){
					try {
						fieldGroup.commit();				
						fireEvent(new IssqnEvent(getUI(), item, true));
		...
		btCancelar = new Button("Cancelar", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(!fieldGroup.isModified()){
					fieldGroup.discard();				
					fireEvent(new IssqnEvent(getUI(), item, false));
	...
	public void addListerner(IssqnListerner target){
		try {
			Method method = IssqnListerner.class.getDeclaredMethod("onClose", IssqnEvent.class);
			addListener(IssqnEvent.class, target, method);
		} catch (Exception e) {
			System.out.println("Método não Encontrado!");
		}
	}
	public void removeListerner(IssqnListerner target){
		removeListener(IssqnEvent.class, target);
	}
	public static class IssqnEvent extends Event{
		
		private Item item;
		private boolean confirm;
		
		public IssqnEvent(Component source, Item item, boolean confirm) {
			super(source);
			this.item = item;
	...
	public interface IssqnListerner extends Serializable{
